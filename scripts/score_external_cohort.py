...
CRITICAL implementation invariant:
  ...
  assumption (see analysis Methods Eq. 1) and the internal-check finding that
...
def compute_singscore(
    ...
    """Full-transcriptome rank-based singscore (matches analysis Methods Eq. 1).
    ...
    if n_genes_total < 5000:
        # Defensive: refuse to score on subsetted matrix (Stage 0 internal-check issue)
        raise ValueError(
    ...
    # Theoretical normalization range (matches analysis Eq. 1)
...
def permutation_p_within_stratum(scores: np.ndarray, labels: np.ndarray, strata: np.ndarray | None,
    ...
    REFUSES to fall back to global shuffle if strata is None. This is the internal-check-
