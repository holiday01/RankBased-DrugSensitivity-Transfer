"""RE-RUN OF THE 36-CELL ABLATION GRID ON THE CORRECTED SCORER (addendum 13.13).

Copied verbatim from revise_bioadv/resubmission_v2/scripts/ablation_grid_completion.py
with exactly two changes, both verifiable by diff:
  1. singscore(): the original computes  norm(up) - norm(1 - down), which expands to
     r_up + r_dn -- the UNDIRECTED rank sum, mathematically identical (Pearson r = 1.000)
     to the undirected score. This is the same C1 sign defect found in the deployed
     phase_6/_common.py scorer, in a second independent script. Corrected to
     norm(up) + norm(1 - down), the bidirectional singscore of Foroutan et al. 2018.
  2. OUT_DIR, redirected to revise_bioadv_386/results/A7b so the original deposit is
     left untouched.

Verification that the correction is right, not just different: the primary configuration
(q=0.30, N=30, OncoKB-all) IS the frozen framework signature, and this run gives its
marginal AUROC on GSE25066 (n=488) as 0.487393. results/A4/A4_within_subtype.json, an
independent script under the corrected scorer, gives 0.4874 for the same quantity. The
pre-correction run gave 0.5954, which no corrected analysis reproduces.
...
REPO = REPO_ROOT / "revise_bioadv"
OUT_DIR = Path("/home/holiday01/2026_ISMB_code/revise_bioadv_386/results/A7b")
