...
ENTRIES = [
    ("GGI",        ROOT / "results/phase_6/GGI_gse25066_residual.json",        "GSE25066"),
    ("RS-like",    ROOT / "results/phase_6/RSlike_gse25066_HRpos_residual.json", "GSE25066 HR+"),
    ("Dox-variant", ROOT / "results/phase_6/DoxVariant_gse25066_residual.json",  "GSE25066"),
    ("Mammaprint", ROOT / "results/phase_6/Mammaprint_metabric_residual.json",  "METABRIC"),
    ("TIS",        ROOT / "results/phase_7/TIS_gse25066_residual.json",         "GSE25066"),
    ("Cytolytic",  ROOT / "results/phase_7/cytolytic_gse22226_residual.json",   "GSE22226"),
    ("DLDA30",     ROOT / "results/phase_7/DLDA30_gse16446_residual.json",      "GSE16446"),
    ("TGFB-strom", ROOT / "results/phase_8/TGFBstrom_gse25066_residual.json",   "GSE25066"),
    ("HRD",        ROOT / "results/phase_8/HRD_gse22226_residual.json",         "GSE22226"),
]
...
def main() -> int:
    ...
    ax_bar.bar(x + width / 2, res_pt, width, yerr=res_err,
               capsize=2, color="#ff7f0e", label="Residual AUROC (after P+H+B)",
               edgecolor="white", lw=0.5,
               error_kw=dict(elinewidth=0.8, ecolor="0.25"))
    ...
    glyph = {"AXIS-EQUIV": "E", "GRAY": "G", "RESIDUAL": "R"}
    for i, r in enumerate(rows):
        short, color = verdict_short(r["verdict"])
        ax_verdict.add_patch(plt.Rectangle((0.05, i - 0.35), 0.9, 0.7,
                                           color=color, ec="black", lw=0.4))
        ax_verdict.text(0.5, i, glyph.get(short, short[:1]),
                        ha="center", va="center",
                        fontsize=10, color="white", fontweight="bold")
    ax_verdict.set_xticks([])
    ax_verdict.set_yticks([])
    ax_verdict.set_title("3x3\nverdict", fontsize=8)
    for spine in ax_verdict.spines.values():
        spine.set_visible(False)
    # legend strip below the verdict col (outside axes)
    fig.text(0.97, 0.04,
             "E = axis-equivalent   G = GRAY zone   R = residual signal",
             ha="right", va="bottom", fontsize=7, color="0.25")

    png = FIG_DIR / "fig5_cross_signature_verdict.png"
    pdf = FIG_DIR / "fig5_cross_signature_verdict.pdf"
